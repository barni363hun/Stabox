...
export class createddatabase1638172481292 implements MigrationInterface {
    name = 'createddatabase1638172481292'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`reciever_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`firstName\` varchar(255) NOT NULL, \`lastName\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`phoneNumber\` varchar(255) NOT NULL, \`adressId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`package_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`postDate\` datetime NOT NULL, \`shippingDate\` datetime NOT NULL, \`size\` varchar(255) NOT NULL, \`weight\` varchar(255) NOT NULL, \`fragile\` tinyint NOT NULL, \`price\` int NOT NULL, \`currentRegion\` int NOT NULL, \`code\` varchar(255) NOT NULL, \`name\` varchar(255) NOT NULL, \`userId\` int NULL, \`shipperId\` int NULL, \`recieverId\` int NULL, \`fromAdressId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`shipper_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`vehicle\` varchar(255) NOT NULL, \`region\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`transaction_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`amount\` int NOT NULL, \`direction\` tinyint NOT NULL, \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`address_entity\` (\`id\` int NOT NULL AUTO_INCREMENT, \`region\` int NOT NULL, \`zipCode\` int NOT NULL, \`cityName\` varchar(255) NOT NULL, \`street\` varchar(255) NOT NULL, \`houseNumber\` int NOT NULL, \`name\` varchar(255) NOT NULL, \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`reciever_entity\` ADD CONSTRAINT \`FK_570418137e0bcb121743f3e4960\` FOREIGN KEY (\`adressId\`) REFERENCES \`address_entity\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`reciever_entity\` DROP FOREIGN KEY \`FK_570418137e0bcb121743f3e4960\``);
